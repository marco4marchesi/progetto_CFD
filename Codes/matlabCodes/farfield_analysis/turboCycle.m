function [CD,CL,CMz,iter,cauchyCD,meshElem] = turboCycle()
%{
...
for idx_T = 1:length(turboNames)
    ...
    [CD.(turboNames(idx_T)),CL.(turboNames(idx_T)),CMz.(turboNames(idx_T)),iter.(turboNames(idx_T)),cauchyCD.(turboNames(idx_T)),meshElem.(turboNames(idx_T))] = orderCycle();

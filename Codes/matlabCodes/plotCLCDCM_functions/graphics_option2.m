...
for idx_T = 1:length(fieldnames(CD))
    ...
    for idx_A = 1:length(fieldnames(CD_TT))
        ...
        for idx_O = 1:length(fieldnames(CD_AA))
            ...
            % CD - plot
            plot(CDaxes,meshElem(2:length(CD_OO)), CD_increment,'o-','Color',lineColors(idx_O),'MarkerFaceColor',faceColors(idx_O))
            hold on;
            xlabel("Nelem")
            ylabel("CD_%")
        ...
        if savePlots
            exportgraphics(CDfigs.(tabName),"IMAGES/CDPlot_"+figureNamer+".pdf")
        end
   end
end
...
for idx_T = 1:length(fieldnames(CL))
    ...
    for idx_A = 1:length(fieldnames(CL_TT))
        ...
        for idx_O = 1:length(fieldnames(CL_AA))
            ...
            % CL - plot
            plot(CLaxes,meshElem(2:length(CL_OO)), CL_increment,'o-','Color',lineColors(idx_O),'MarkerFaceColor',faceColors(idx_O))
            hold on;
            xlabel("Nelem")
            ylabel("CL_%")
            legend('1st order','2nd order')
        ...
        if savePlots
            exportgraphics(CLfigs.(tabName),"IMAGES/CLPlot_"+figureNamer+".pdf")
        end
    end
end
...
for idx_T = 1:length(fieldnames(CMz))
    ...
    for idx_A = 1:length(fieldnames(CMz_TT))
        ...
        for idx_O = 1:length(fieldnames(CMz_AA))
            ...
            % CMz - plot
            plot(CMzaxes,meshElem(2:length(CMz_OO)), CMz_increment,'o-','Color',lineColors(idx_O),'MarkerFaceColor',faceColors(idx_O))
            hold on;
            xlabel("Nelem")
            ylabel("CMz_%")
            legend('1st order','2nd order')
        ...
        if savePlots
            exportgraphics(CMzfigs.(tabName),"IMAGES/CMzPlot_"+figureNamer+".pdf")
        end
